...
class AirbyteExampleStream(HttpStream, ABC):
    """
    ...
    `class AirbyteExampleStream(HttpStream, ABC)` which is the current class
    `class Customers(AirbyteExampleStream)` contains behavior to pull data for customers using v1/customers
    `class Employees(AirbyteExampleStream)` contains behavior to pull data for employees using v1/employees

    If some streams implement incremental sync, it is typical to create another class
    `class IncrementalAirbyteExampleStream((AirbyteExampleStream), ABC)` then have concrete stream implementations extend it. An example
    is provided below.
...
class Customers(AirbyteExampleStream):
    """
...
class IncrementalAirbyteExampleStream(AirbyteExampleStream, ABC):
    """
...
class Employees(IncrementalAirbyteExampleStream):
    """
    TODO: Change class name to match the table/data source this stream corresponds to.
    """
...
# Source
class SourceAirbyteExample(AbstractSource):
    def check_connection(self, logger, config) -> Tuple[bool, any]:

        base_url = "http://api.exchangeratesapi.io/v1/latest?"
        query = base_url + "access_key=" + config["access_key"]
        try:
    ...
    def streams(self, config: Mapping[str, Any]) -> List[Stream]:
        #access_key="23a71b68624b50485ecdf5f27b8c2f31"
        auth = NoAuth()
        ...
        #auth = TokenAuthenticator(token="api_token")  # Oauth2Authenticator is also available if you need oauth support
...
class ExchangeRates(HttpStream):
    ...
    def path(self, stream_state: Mapping[str, Any] = None, 
        ...
        return stream_slice['date'] + "?access_key=23a71b68624b50485ecdf5f27b8c2f31" 
        #return "latest?access_key=23a71b68624b50485ecdf5f27b8c2f31"  
